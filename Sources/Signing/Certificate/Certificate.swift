...
#if os(macOS)
public typealias Certificate = CoreCertificate
#elseif os(Linux) || os(Windows) || os(Android)
public typealias Certificate = BoringSSLCertificate
#else
public typealias Certificate = UnsupportedCertificate
#endif

// MARK: - Certificate implementation using the Security framework

#if os(macOS)
public struct CoreCertificate {
    ...
    public func subject() throws -> CertificateName {
        try self.extractName(kSecOIDX509V1SubjectName)
    }

    public func issuer() throws -> CertificateName {
    ...
    public func keyType() throws -> KeyType {
        guard let key = SecCertificateCopyKey(self.underlying) else {
            throw CertificateError.keyExtractionFailure
        }
...
#elseif os(Linux) || os(Windows) || os(Android)
public final class BoringSSLCertificate {
    #if CRYPTO_v2
...
#else
public struct UnsupportedCertificate {
    public init(derEncoded data: Data) throws {
